...
//            conn.close();
    ...
    public void setSaveLabBilling(String branch_id,String ltb_id, String flag){
        // step ในการนำเข้า lab ประจำงวด
        // 1. นำเข้าข้อมูลจาก excel ก่อน
        //    นำเข้าข้อมูลจาก excel โดยตรงเพื่อจะได้ข้อมูลทั้งหมด
        // 2. update รายการ lab ที่ยังไม่มีใน master lab_b_item เพราะในแต่ละสาขาอาจเพิ่มรายการ lab ได้เอง
        //    และเป็นโปรแกรมเพื่อออก bill เท่านั้น ไม่ใช่โปรแกรมหลัก
        // 3. Check ราคา net_price ให้ถูกต้องตามเงื่อนไข เพราะ ราคาที่ส่งมาให้จะเป็นราคา master ต้องมาหาราคาใหม่เอง
        ltb = getLabTBillingByPK(branch_id, ltb_id, "");
        Vector v_sheet = openWorkBook(branch_id,ltb.getFilename(),ltb.getLabBillingId(), flag);//ดึงข้อมูลจาก excel
        setSaveWorkBook(branch_id, ltb_id, v_sheet, flag);//save ลง table
        //ไม่ต้อง สร้างใน lab master แล้ว คือ ทำให้โปรแกรมช้า และไม่ถูกต้อง คือถ้ามีการแก้ไขราคาหรืออะไร ก็ควรเป็นข้อมูลของครั้งๆนั้นไปเลย คือให้เก็บใน lab_t_billing_item ดีกว่า
//        setUpdateLabBItem(branch_id, ltb_id);// update lab_b_item ในแต่ละสาขา
        ...
        try{
            ...
            if(c_fname != null && (c_fname.getCellType()==0)){//numeric
                if(flag.equals("date")){
                    ...
                    year = date.getYear()+1900;//ต้องบวกอีก 1900 เพราะตาม javadoc บอกแบบนี้
...
//                paidNamet="";
                ...
                if(!ltb_i.getPatientFullname().equals("")){//ต้องทำอย่างนี้เพราะใน excel จะไม่มี fullname เมื่อ เป็นข้อมูลของ hn เดิม แต่ตรวจหลายอย่าง
                    fullNamet = ltb_i.getPatientFullname();
                }
                if(!ltb_i.getVisitHn().equals("")){//ต้องทำอย่างนี้เพราะใน excel จะไม่มี visithn เมื่อ เป็นข้อมูลของ hn เดิม แต่ตรวจหลายอย่าง
                    visitHn = ltb_i.getVisitHn();
                }
                if(!ltb_i.getOrderDate().equals("")){//ต้องทำอย่างนี้เพราะใน excel จะไม่มี order date เมื่อ เป็นข้อมูลของ hn เดิม แต่ตรวจหลายอย่าง
                    orderDate = ltb_i.getOrderDate();
                }
                if(!ltb_i.getPaidNamet().equals("")){//ต้องทำอย่างนี้เพราะใน excel จะไม่มี paid name เมื่อ เป็นข้อมูลของ hn เดิม แต่ตรวจหลายอย่าง
                    paidNamet = ltb_i.getPaidNamet();
                }
                if(!ltb_i.getDocId().equals("")){//ต้องทำอย่างนี้เพราะใน excel จะไม่มี vn เมื่อ เป็นข้อมูลของ hn เดิม แต่ตรวจหลายอย่าง
    ...
    private String setStatusDiscount(String labNamet){
        if( setStatusOutLab(labNamet).equals("2")){//ถ้าเป็น out lab ไม่ให้ส่วนลด
            return "2";
        }else{
            return "1";
        }
...
//            lbi = new LabBItem();
    ...
    private Double getLabNetPrice(Statement st,String branchId, String statusOutLab, String statusDiscount,
        ...
        // step ในการ update ราคา
        //1.  check lab number ว่าเป็น out lab หรือไม่
        //    ถ้า out lab ก็ไม่ต้องลดราคา ใช้ price_sale2 ได้เลย
        //2.  ถ้า ไม่เป็น out lab ก็ให้ check ต่อว่ามีส่วนลดจาก ประเภท lab หรือไม่
        //    โดย check จาก  lab type ว่าประเภทนี้มีส่วนลดเป็น bahstatusOutLabt หรือ percent
        Double netPrice = 0.0;
//        ltb_i = getLabTBillingItemByPK(st,branchId, ltbIId,"");//ดึงข้อมูล ทั้งหมดที่ต้องการ update ราคา
//        if(!ltb_i.getLabBillingItemId().equals("")){
//            lbi = getLabBItemByPK(branchId, ltb_i.getItemNumber(),"lab_number");

        if(statusOutLab.equals("1")){//ทำเอง
            
            if(statusDiscount.equals("2")){//ใช้ persent
...
//        }
    ...
    private void setUpdateNetPrice(String branchId, String ltbId){
        ...
        try {
            ...
            vLtb = getLabTBillingItem(branchId, ltbId, "","lab_t_billing");//ต้องดึงตามนี้ เพราะดึงตาม group by แล้วอาจจะมี item_number ที่มีหลายสิทธิก็ได้
            for(int i=0; i<=vLtb.size()-1;i++){
                remark="";
                discountBaht=0.0;
                discountPer = 0.0;
                ...
                if(ltb_i.getItemCommonName().indexOf("outlab")>0 || (ltb_i.getItemCommonName().indexOf("Out lab")>0) ||
                    ...
                    netPrice = ltb_i.getPriceSale3();//ไม่ให้ส่วนลด
                }else{
                    statusOutLab = "1";
                    ...
                    if(lbp.getStatusdiscount().equals("1")){// discount baht
                        ...
                        //remark += " ส่วนลดเป็น baht "+ltb_i.getPriceSale2().toString()+" - "+lbp.getDiscount().toString();
                        remark += " ส่วนลดเป็น baht "+ltb_i.getPriceSale3().toString()+" - "+lbp.getDiscount().toString();
                        ...
                        //remark += " ส่วนลดเป็น percent "+ltb_i.getPriceSale3().toString()+" - ("+lbp.getDiscount().toString()+" * "+ltb_i.getPriceSale3()+"/100)";
                        remark += " ส่วนลดเป็น percent "+ltb_i.getPriceSale2().toString()+" - ("+lbp.getDiscount().toString()+" * "+ltb_i.getPriceSale2()+"/100)";
...
//            conn.close();
    ...
    public String getTrLabBillingItemGroupPaidNamet(String branch_id, String ltbId){
        ...
        txtH = "<table><tr><th align='center'>ลำดับ</th><th>ประเภท</th><th align='right'>จำนวน</th><th align='right'>มูลค่า</th></tr>";
    ...
    public String getTrLabBillingItemGroupItemNumber(String branch_id, String ltbId){
        ...
        txtH = "<table><tr><th>ลำดับ</th><th>ประเภท</th><th>จำนวน</th><th align='right'>มูลค่า</th></tr>";
    ...
    public String getTrLabTBillingView(String branchId, String yearId){
        ...
        txtH = "<table widht='100%'><tr><th>ลำดับ</th><th>เลขที่</th><th width='300'>รายการ</th><th>เดือน</th><th>งวด</th><th>จำนวน</th><th>มูลค่า</th><th>ไม่พบรหัสlab</th><th>ไม่พบรหัสhn</th></tr>";
        txtF="</table>";
        return txtH+txt+txtF;
    }
...
//        Integer i=0;
        ...
        txtH = "<table><tr><th align='center'>ลำดับ</th><th>ชื่อ-นามสกุล</th><th align='right'>รหัส</th>"
                + "<th align='right'>รายการ</th><th align='right'>price2</th><th align='right'>ราคาสุทธิ</th></tr>";
        txtF = "</table>";
        return txtH+txt+txtF;
    }
